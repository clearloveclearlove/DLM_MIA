...
class MIARunner:
    ...
    def _run_attacks_on_dataset(self, dataset, ds_name):
        ...
        for attack_name, attack_module in self.config_manager.get_available_attacks().items():
            ...
            # Compute metrics
            metrics_names = processed_dataset.column_names[2:]
            y_true = processed_dataset['label']
            
            for metrics_name in  metrics_names:
                y_score = processed_dataset[metrics_name]
                ...
                attack_results.append([f'{attack_name}_{metrics_name}'] + metrics)
                logging.info(f"Attack f'{attack_name}_{metrics_name}' results: {metrics}")
...
args = parse_args()
args.config = "attack/configs/config_fond.yaml"
args.output = "out/fond"
